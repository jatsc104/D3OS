...
pub fn set_up_rx_desc_ring(registers: &E1000Registers) -> Vec<E1000RxDescriptor>{
    ...
    const RECEIVE_RING_SIZE: usize = 128; //has to be multiple of 128
    const BUFFER_SIZE: usize = 2048; //can be 256, 512, 1024, 2048, 4096, 8192, 16384 Bytes
    ...
    rdtr = rdtr | 0x00000000;   //set to 0 - triggers an interrupt immediately after packet is received
...
//This funtion is currently not used but is kept for future refactoring as a feature
#[allow(dead_code)]
...
///Initialises a given transmit descriptor ring with size TX_NUM_DESCRIPTORS, which is set in e1000_driver.rs
...
///Copies data from a given TxBuffer and marks the corresponding descriptors as ready to transmit
...
///Function isn't used anymore, but is kept for future reference, since some parts differ from the vecless version
#[allow(dead_code)]
...
///This Function splits up data into packets of maximum size 16288 bytes and copies it into memory usable by the NIC
...
//This is the old version of the function, which is kept for future reference
#[allow(dead_code)]
...
///This function calculates the size of the header of a given TxBuffer.
///It isn't used in the current implementation, but is kept as part of a possible future feature to move some of the Paket fragmentation logic to the driver instead of a future network stack
#[allow(dead_code)]
...
///This function retrieves ALL Pakets marked as ready from the receive ring and enqueues them into the receive buffer.
///This function ignores the head and tail pointers and processes ALL packets in the ring
...
///This function is essentially a wrapper for the try_enqueue function of the bounded queue
...
///This function retreives the "oldest" packet from the receive ring and enqueues it into the receive buffer
